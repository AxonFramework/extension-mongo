...
class MongoTrackingTokenTest {
    ...
    void testAdvanceToSameIdentifierNotAllowed() {
        assertThrows(Exception.class, () -> MongoTrackingToken.of(time(0), "0").advanceTo(time(1), "0", ONE_SECOND));
    }
    ...
    void testAdvanceToPriorIdentifierNotAllowed() {
        assertThrows(
                Exception.class,
                () -> MongoTrackingToken.of(time(0), "1")
                                        .advanceTo(time(1), "2", ONE_SECOND)
                                        .advanceTo(time(2), "1", ONE_SECOND)
        );
    }
